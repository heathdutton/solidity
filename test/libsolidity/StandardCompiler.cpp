...
BOOST_AUTO_TEST_CASE(no_experimental_invalid_output_selection)
{
	frontend::StandardCompiler compiler;
	{
		...
		BOOST_REQUIRE(util::jsonParseStrict(input, parsedInput));
		Json result = compiler.compile(parsedInput);
		BOOST_CHECK(containsError(result, "FatalError", "'irAst', 'irOptimizedAst' and 'yulCFGJson' outputs are experimental, and can only be used by toggling the 'settings.experimental' option."));
	}
	{
		char const* input = R"(
		{
...
BOOST_AUTO_TEST_CASE(no_experimental_ethdebug)
{
	...
	BOOST_REQUIRE(util::jsonParseStrict(input, parsedInput));
	Json result = compiler.compile(parsedInput);
	BOOST_CHECK(containsError(result, "FatalError", "'settings.debug.debugInfo.ethdebug' is experimental, and can only be used by toggling the 'settings.experimental' option."));
}
